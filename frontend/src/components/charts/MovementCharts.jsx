...
export function MovementScoreGauge({ score = 0, size = 180 }) {
  ...
  return <div className="relative mx-auto" style={{ width: size, height: size }} role="img" aria-label={`${t("results.movementScore")} ${bounded} ${t("chart.outOf100")}`}><svg viewBox="0 0 160 160" className="-rotate-90"><circle cx="80" cy="80" r={radius} fill="none" stroke="var(--chart-track)" strokeWidth="13" /><circle cx="80" cy="80" r={radius} fill="none" stroke="url(#scoreGradient)" strokeWidth="13" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={circumference * (1 - bounded / 100)} /><defs><linearGradient id="scoreGradient"><stop stopColor="#2563eb" /><stop offset="1" stopColor="var(--brand-teal)" /></linearGradient></defs></svg><div className="absolute inset-0 grid place-content-center text-center"><span className="text-4xl font-bold text-clinical-ink">{bounded}</span><span className="text-xs font-semibold uppercase tracking-wider text-slate-400">{t("chart.outOf100")}</span></div></div>;
}
...
export function MovementRadarChart({ report }) {
  ...
  return <div><svg viewBox="0 0 240 240" className="mx-auto h-60 w-full max-w-xs" role="img" aria-label={t("chart.profileAria")}><g fill="none" stroke="var(--chart-grid)">{[.25, .5, .75, 1].map((scale) => <polygon key={scale} points={values.map((_, index) => vertex(index, scale).join(",")).join(" ")} />)}{values.map((_, index) => <line key={index} x1={center} y1={center} x2={vertex(index)[0]} y2={vertex(index)[1]} />)}</g><polygon points={polygon} fill="rgba(37,99,235,.22)" stroke="#60a5fa" strokeWidth="2.5" />{labels.map((label, index) => { const [x, y] = vertex(index, 1.18); return <text key={label} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fill="var(--chart-label)" fontSize="10">{label}</text>; })}</svg><p className="text-center text-[11px] leading-5 text-slate-500">{t("chart.normalized")}</p></div>;
}
