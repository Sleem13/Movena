...
export function MovementScoreGauge({ score = 0, size = 180 }) {
  ...
  return <div className="relative mx-auto" style={{ width: size, height: size }} role="img" aria-label={`Movement score ${bounded} out of 100`}><svg viewBox="0 0 160 160" className="-rotate-90"><circle cx="80" cy="80" r={radius} fill="none" stroke="#e8eef3" strokeWidth="13" /><circle cx="80" cy="80" r={radius} fill="none" stroke="url(#scoreGradient)" strokeWidth="13" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={circumference * (1 - bounded / 100)} /><defs><linearGradient id="scoreGradient"><stop stopColor="#2563eb" /><stop offset="1" stopColor="#0f8f83" /></linearGradient></defs></svg><div className="absolute inset-0 grid place-content-center text-center"><span className="text-4xl font-bold text-clinical-ink">{bounded}</span><span className="text-xs font-semibold uppercase tracking-wider text-slate-400">out of 100</span></div></div>;
}
...
export function IssueBreakdownChart({ report }) {
  ...
  if (!entries.length) return <EmptyState compact title="No issue breakdown" description="No movement issues were returned in this analysis." icon={ChartNoAxesColumnIncreasing} />;
  const max = Math.max(...entries.map(([, count]) => count));
  return <div className="space-y-4">{entries.map(([issue, count]) => <div key={issue}><div className="mb-1.5 flex justify-between gap-3 text-xs"><span className="font-semibold text-slate-700">{pretty(issue)}</span><span className="text-slate-500">{frameIssues.length ? `${count} sampled frames` : "Detected"}</span></div><div className="h-2.5 rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-amber-400 to-orange-500" style={{ width: `${Math.max(12, count / max * 100)}%` }} /></div></div>)}</div>;
}

export function MovementRadarChart({ report }) {
  const values = [Math.max(0, Math.min(100, report.movement_score || 0)), Math.min(100, (report.average_knee_angle || 0) / 1.8), Math.min(100, (report.average_hip_angle || 0) / 1.8), Math.max(0, 100 - (report.average_trunk_angle || 0) / 0.9), Math.max(0, 100 - (report.detected_issues?.length || 0) * 20)];
  const labels = ["Score", "Knee", "Hip", "Trunk", "Issue load"];
  ...
  return <div><svg viewBox="0 0 240 240" className="mx-auto h-60 w-full max-w-xs" role="img" aria-label="Descriptive movement profile radar"><g fill="none" stroke="#dce6ee">{[.25, .5, .75, 1].map((scale) => <polygon key={scale} points={values.map((_, index) => vertex(index, scale).join(",")).join(" ")} />)}{values.map((_, index) => <line key={index} x1={center} y1={center} x2={vertex(index)[0]} y2={vertex(index)[1]} />)}</g><polygon points={polygon} fill="rgba(37,99,235,.18)" stroke="#2563eb" strokeWidth="2.5" />{labels.map((label, index) => { const [x, y] = vertex(index, 1.18); return <text key={label} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fill="#64748b" fontSize="10">{label}</text>; })}</svg><p className="text-center text-[11px] leading-5 text-slate-500">Normalized descriptive profile for this session—not a validated clinical scale.</p></div>;
}

export function RepQualityChart({ reps = [] }) {
  if (!reps.length) return <EmptyState compact title="Rep-level quality is planned" description="The current API returns total repetitions but not individual rep scores." icon={ChartNoAxesColumnIncreasing} />;
  return <div className="flex h-44 items-end gap-3 rounded-xl bg-slate-50 p-4">{reps.map((rep, index) => <div key={rep.rep_index ?? index} className="flex flex-1 flex-col items-center gap-2"><div className="w-full rounded-t-lg bg-gradient-to-t from-clinical-blue to-clinical-teal" style={{ height: `${Math.max(8, rep.score || 0)}%` }} /><span className="text-[11px] font-semibold text-slate-500">Rep {index + 1}</span></div>)}</div>;
}
