...
export function RepCountReliabilityNotice({ report }) {
  ...
  return <Card className="p-5"><div className="flex flex-wrap items-center gap-3"><Activity size={19} className="text-amber-600" aria-hidden="true" /><h2 className="font-bold text-clinical-ink">Rep count review</h2>{lowConfidence && <Badge tone="amber">Manual review recommended</Badge>}</div><p className="mt-3 text-sm leading-6 text-slate-600">{lowConfidence ? "The movement was detected, but rep count confidence is low due to noisy pose tracking or inconsistent visibility." : `${partials} meaningful incomplete movement cycle${partials === 1 ? " was" : "s were"} ignored and did not change the completed rep count.`}</p>{lowConfidence && <p className="mt-2 text-xs leading-5 text-slate-500">Review camera setup and consider trimming the video to only the squat set.</p>}</Card>;
}
...
export function RejectedAnalysisCard({ report, onAnalyzeAnother }) {
  ...
  return <div className="space-y-5"><Card className="border-amber-200 p-6"><div className="flex items-start gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-amber-100 text-amber-700"><AlertTriangle size={23} aria-hidden="true" /></span><div><Badge tone="amber">Recording needs review</Badge><h2 className="mt-3 text-2xl font-bold text-clinical-ink">No valid squat movement detected</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{report.message || "The recording did not contain enough visible, complete squat movement to produce a reliable score."}</p></div></div>{warnings.length > 0 && <ul className="mt-5 space-y-2 rounded-xl bg-amber-50 p-4 text-sm text-amber-950">{warnings.map((warning) => <li key={warning} className="flex gap-2"><span aria-hidden="true">•</span><span>{warning}</span></li>)}</ul>}<div className="mt-5 rounded-xl border border-blue-100 bg-blue-50 p-4"><p className="font-semibold text-clinical-ink">Try recording again</p><p className="mt-1 text-sm leading-6 text-slate-600">Record full body, 3–5 squat reps, stable camera, good lighting.</p></div><Button className="mt-5" type="button" onClick={onAnalyzeAnother}><RotateCcw size={16} aria-hidden="true" />Upload another video</Button></Card><CameraGuide /><LimitationsCard limitations={report.limitations} /></div>;
}
