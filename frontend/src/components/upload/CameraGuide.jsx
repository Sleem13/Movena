...
export default function CameraGuide({ exercise = "bodyweight_squat", metadata }) {
  ...
  return <Card className="overflow-hidden"><div className="bg-gradient-to-br from-clinical-navy to-clinical-blue p-6 text-white"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-white/15"><Camera size={22} aria-hidden="true" /></span><div><p className="text-xs font-semibold uppercase tracking-widest text-blue-100">Recording tips</p><h2 className="mt-1 text-xl font-bold">Camera placement guide</h2></div></div>{metadata && <p className="mt-4 text-sm text-blue-50"><strong>{metadata.recommended_camera_view}.</strong> Keep {metadata.required_landmarks?.join(", ")} visible.</p>}<div className="mt-6 grid grid-cols-2 gap-3"><div className="rounded-xl bg-white/10 p-3"><ScanLine size={18} aria-hidden="true" /><p className="mt-2 text-xs leading-5 text-blue-50">Keep the lens near the movement area and all required joints fully framed.</p></div><div className="rounded-xl bg-white/10 p-3"><Lightbulb size={18} aria-hidden="true" /><p className="mt-2 text-xs leading-5 text-blue-50">Use good lighting; avoid strong backlight and moving backgrounds.</p></div></div></div><ul aria-label="Visibility checklist" className="grid gap-1 p-5">{tips.map(([title, text]) => <li key={title} className="flex gap-3 rounded-xl px-2 py-2.5"><CheckCircle2 className="mt-0.5 shrink-0 text-clinical-teal" size={17} aria-hidden="true" /><div><p className="text-sm font-semibold text-slate-800">{title}</p><p className="mt-0.5 text-xs leading-5 text-slate-500">{text}</p></div></li>)}</ul></Card>;
}
