...
export default function VerifyEmail({ onLogin }) {
  ...
  async function resend(event) {
    ...
    try {
      setMessage((await resendVerificationEmail(email)).message);
    } catch (requestError) {
      setError(
        getApiErrorMessage(
          requestError,
  ...
  return (
    <AuthRecoveryLayout onLogin={onLogin}>
      <ShieldCheck className="text-clinical-teal" size={30} />
      <h1 className="mt-4 text-3xl font-extrabold text-clinical-ink">
        Verify your email
      </h1>
      {status === "verifying" ? (
        <div className="mt-6">
          <LoadingSpinner label="Verifying secure link" />
        </div>
      ) : null}
      {message ? (
      ...
      {status === "verified" ? (
        ...
        <form onSubmit={resend} className="mt-6 grid gap-4">
          ...
          <label className="relative">
            ...
            <input
              ...
              required
              autoComplete="email"
              className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-11 pr-3 text-sm"
