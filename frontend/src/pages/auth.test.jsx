...
it("renders login and development privacy warning",()=>{render(<Login/>);expect(screen.getByRole("heading",{name:"Log in to PhysioVision"})).toBeInTheDocument();expect(screen.getByText(/do not enter real patient-identifiable data/i)).toBeInTheDocument();});
it("renders public registration without privileged role selection",()=>{render(<Register/>);expect(screen.getByRole("heading",{name:"Create a development account"})).toBeInTheDocument();expect(screen.queryByText("admin")).not.toBeInTheDocument();});
