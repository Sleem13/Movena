...
import { CalendarClock, Check, Download, ExternalLink, GitCompareArrows, History, Radio, RefreshCw, UploadCloud, X } from "lucide-react";
...
const HISTORY_EXERCISES = ["bodyweight_squat", "sit_to_stand", "knee_extension", "shoulder_abduction", "shoulder_flexion", "hip_abduction", "walking_gait_screen", "balance", "push_up", "shoulder_press", "bicep_curl", "hammer_curl"];
...
export default function SessionHistory({ onAnalyze, onCoach }) {
  ...
  const comparisonSessions = comparisonIds.map((id) => sessions.find((item) => item.session_id === id)).filter(Boolean);

  function toggleComparison(session) {
    setComparisonError("");
    setComparisonDetails([]);
    setComparisonIds((current) => {
      if (current.includes(session.session_id)) return current.filter((id) => id !== session.session_id);
  ...
  useEffect(() => {
    ...
    Promise.all(comparisonIds.map((id) => getSavedSession(id)))
      .then((items) => { if (active) setComparisonDetails(items); })
      .catch(() => { if (active) setComparisonError(t("history.compareError")); })
  ...
  return (
    <main>
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-3 lg:flex lg:flex-wrap lg:items-end">
        <div className="min-w-48"><label className="mb-1.5 block text-xs font-semibold text-slate-600" htmlFor="history-exercise">{t("common.exercise")}</label><Select id="history-exercise" ariaLabel={t("common.exercise")} value={exercise} onChange={setExercise} options={[{ value: "", label: t("history.allExercises") }, ...HISTORY_EXERCISES.map((item) => ({ value: item, label: exerciseText(item).name }))]} /></div>
        <div className="min-w-40"><label className="mb-1.5 block text-xs font-semibold text-slate-600" htmlFor="history-status">{t("common.status")}</label><Select id="history-status" ariaLabel={t("common.status")} value={status} onChange={setStatus} options={[{ value: "", label: t("history.allStatuses") }, { value: "success", label: t("common.success") }, { value: "rejected", label: t("common.rejected") }]} /></div>
        <div className="min-w-40"><label className="mb-1.5 block text-xs font-semibold text-slate-600" htmlFor="history-sort">{t("common.date")}</label><Select id="history-sort" ariaLabel={t("common.date")} value={sortOrder} onChange={setSortOrder} options={[{ value: "newest", label: t("history.newest") }, { value: "oldest", label: t("history.oldest") }]} /></div>
      </div>
      {error && <Alert title={t("history.unavailable")}>{error}</Alert>}
      <SessionComparison selected={comparisonSessions} details={comparisonDetails} loading={comparisonLoading} error={comparisonError} onClear={() => { setComparisonIds([]); setComparisonDetails([]); setComparisonError(""); }} />
      {loading ? (
        <Card className="grid min-h-48 place-items-center"><LoadingSpinner label={t("history.loading")} /></Card>
      ) : sessions.length === 0 ? (
        <EmptyState title={t("history.emptyTitle")} description={t("history.emptyDescription")} icon={History} actions={<><Button onClick={onAnalyze}><UploadCloud size={16} />{t("nav.analyzeMovement")}</Button>{onCoach ? <Button variant="secondary" onClick={onCoach}><Radio size={16} />{t("nav.coach")}</Button> : null}</>} />
      ) : (
          ...
          {visibleSessions.map((session) => (
            <Card key={session.session_id} className="rounded-none border-0 border-b border-slate-200 p-5 shadow-none ring-0 last:border-b-0 hover:bg-slate-50/70">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Button
                  variant={comparisonIds.includes(session.session_id) ? "primary" : "secondary"}
                  onClick={() => toggleComparison(session)}
                  disabled={(comparisonIds.length >= 2 && !comparisonIds.includes(session.session_id)) || Boolean(comparisonSessions[0] && comparisonSessions[0].exercise_id !== session.exercise_id)}
                  aria-pressed={comparisonIds.includes(session.session_id)}
                  title={comparisonSessions[0] && comparisonSessions[0].exercise_id !== session.exercise_id ? t("history.compareSameExercise") : undefined}
                >
                  {comparisonIds.includes(session.session_id) ? <Check size={15} /> : <GitCompareArrows size={15} />}{t("history.compareAction")}
                </Button>
      ...
      {artifact.status !== "idle" && createPortal(
        <div className="fixed inset-0 z-[100] overflow-y-auto p-4" role="dialog" aria-modal="true" aria-label={artifact.kind === "report" ? t("history.reportViewer") : t("history.overlayViewer")}>
          <button type="button" className="fixed inset-0 cursor-default bg-slate-950/60 backdrop-blur-[2px]" onClick={closeArtifact} aria-label={t("common.close")} />
          ...
          <Card className="relative z-10 flex max-h-[calc(100vh-2rem)] w-full max-w-4xl flex-col overflow-hidden rounded-2xl shadow-2xl">
            <div className="flex shrink-0 items-center justify-between border-b border-slate-100 bg-white p-4">
              ...
              <button ref={artifactCloseRef} type="button" onClick={closeArtifact} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-clinical-ink focus-visible:ring-2 focus-visible:ring-blue-400" aria-label={t("common.close")}><X size={20} /></button>
            </div>
