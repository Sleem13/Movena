...
function ExercisePickerDialog({ exercises, onChoose, onClose }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[120] grid place-items-center overflow-y-auto bg-slate-950/55 p-4 backdrop-blur-sm" onMouseDown={onClose}>
      <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="exercise-picker-title" tabIndex={-1} className="my-auto w-full max-w-3xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_30px_90px_rgba(7,20,38,0.28)] outline-none" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 bg-gradient-to-r from-white via-blue-50/40 to-teal-50/40 p-6 sm:p-7">
          <div className="flex gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-200"><Dumbbell size={23} aria-hidden="true" /></span><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-clinical-teal">{t("upload.pickerEyebrow")}</p><h2 id="exercise-picker-title" className="mt-1 text-2xl font-extrabold tracking-tight text-clinical-ink">{t("upload.pickerTitle")}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-600">{t("upload.pickerDescription")}</p></div></div>
          <button type="button" onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700" aria-label={t("common.close")}><X size={19} /></button>
        </div>
        <div className="grid max-h-[60vh] gap-3 overflow-y-auto p-5 sm:grid-cols-2 sm:p-6">
          {exercises.map((exercise) => {
            const text = exerciseText(exercise.exercise_id);
            return <button key={exercise.exercise_id} type="button" onClick={() => onChoose(exercise.exercise_id)} className="group flex min-h-28 items-start gap-4 rounded-2xl border border-slate-200 bg-white p-4 text-start transition hover:-translate-y-0.5 hover:border-blue-300 hover:bg-blue-50/40 hover:shadow-lg hover:shadow-blue-100 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-100"><span className="mt-0.5 grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-500 transition group-hover:bg-blue-600 group-hover:text-white"><Dumbbell size={18} /></span><span className="min-w-0 flex-1"><span className="block font-bold text-clinical-ink">{text.name}</span><span className="mt-1 block text-xs font-semibold uppercase tracking-wide text-clinical-teal">{text.bodyRegion}</span><span className="mt-2 block text-xs leading-5 text-slate-500">{text.cameraView}</span></span><Check size={18} className="mt-2 shrink-0 text-transparent transition group-hover:text-clinical-teal" /></button>;
          })}
        </div>
      </section>
    </div>,
...
export default function UploadSquat(props) {
  ...
  const selectedExercise = exercises.find((item) => item.exercise_id === props.exercise);
  const selected = selectedExercise ? translatedExercise(selectedExercise, exerciseText) : null;
  const selectedName = props.exercise ? exerciseText(props.exercise) : { short: t("upload.movement") };
  ...
  return (
    <main>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[1fr_380px]">
        <div className="space-y-5">
          <Card className="p-5">
            ...
            <Select
              ...
              options={[
                { value: "", label: t("upload.chooseExercise"), disabled: true },
            ...
            />
            {selected ? <div className="mt-4 grid gap-3 rounded-xl bg-blue-50 p-4 text-sm sm:grid-cols-2">
              <div>
            ...
            </div> : <button type="button" onClick={() => setShowExercisePicker(true)} className="mt-4 flex w-full items-center gap-3 rounded-xl border border-dashed border-blue-300 bg-blue-50/40 p-4 text-start transition hover:border-blue-400 hover:bg-blue-50"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-clinical-blue shadow-sm"><Dumbbell size={18} /></span><span><span className="block text-sm font-bold text-clinical-ink">{t("upload.noExerciseTitle")}</span><span className="mt-1 block text-xs leading-5 text-slate-500">{t("upload.noExerciseDescription")}</span></span></button>}
          </Card>
          {showRecognition ? (
          ...
          {!desktopLayout && selected ? <CameraGuide exercise={props.exercise} metadata={selected} compact /> : null}
        ...
        {desktopLayout && selected ? <CameraGuide exercise={props.exercise} metadata={selected} /> : null}
      </div>
      {showExercisePicker ? <ExercisePickerDialog exercises={supported} onChoose={chooseExercise} onClose={() => setShowExercisePicker(false)} /> : null}
    </main>
  );
}
