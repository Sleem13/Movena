...
def test_include_overlay_returns_preview_and_download_urls(monkeypatch, tmp_path):
    ...
    overlay_path = tmp_path / "artifacts/overlays/abc.webm"
...
def test_include_overlay_does_not_return_urls_when_file_is_missing(monkeypatch, tmp_path):
    setup_analysis(monkeypatch, tmp_path)
    missing_path = tmp_path / "artifacts/overlays/missing.webm"
    monkeypatch.setattr(
        "app.api.routes.squat_analysis.create_overlay_video",
        lambda *_args: OverlayArtifact(
            overlay_id="missing",
...
def test_overlay_preview_and_download_routes_return_browser_compatible_webm(monkeypatch, tmp_path):
    artifact_root = tmp_path / "artifacts"
    monkeypatch.setattr(get_settings(), "artifact_dir", artifact_root)
    overlay_id = "0123456789abcdef0123456789abcdef"
    overlay = artifact_root / "overlays" / f"{overlay_id}.webm"
    overlay.parent.mkdir(parents=True)
    overlay.write_bytes(b"webm-test")
    ...
    suffix_preview = client.get(f"/api/v1/artifacts/overlays/{overlay_id}.webm/preview")
    ...
    assert preview.content == b"webm-test"
