    ...
    try:
        ...
        if include_overlay and settings.enable_overlay_generation and report.status == "success" and report.frame_analysis:
            try:
                ...
                logger.warning("%s overlay generation failed: %s", analyzer.exercise_id, exc)
        ...
        if save_session and settings.enable_session_history and can_save:
            try:
                saved = save_analysis_session(
                    ...
                    media_metadata={"content_type": video.content_type, "size_bytes": video_path.stat().st_size if video_path.exists() else None},
        ...
        return error_response(status.HTTP_500_INTERNAL_SERVER_ERROR, "PROCESSING_ERROR", f"Unable to complete {analyzer.config.display_name.lower()} analysis.")
    finally:
        remove_file(video_path)


@router.post("/push-up", response_model=AnalysisResponse, responses={400: {"model": ErrorResponse}, 422: {"model": ErrorResponse}, 500: {"model": ErrorResponse}})
async def analyze_push_up(
...
async def analyze_shoulder_press(
    video: UploadFile = File(...),
    include_overlay: bool = Query(False),
    include_frame_data: bool = Query(False),
    generate_report: bool = Query(False),
