"""Upload endpoint for the rule-based shoulder-abduction analyzer."""
...
@router.post("/shoulder-abduction", response_model=AnalysisResponse, responses={400: {"model": ErrorResponse}, 422: {"model": ErrorResponse}, 500: {"model": ErrorResponse}})
async def analyze_shoulder_abduction(video: UploadFile = File(...), include_overlay: bool = Query(False), include_frame_data: bool = Query(False), generate_report: bool = Query(False), include_ml: bool = Query(False), continue_on_subject_warning: bool = Query(False), save_session: bool = Query(False), patient_id: str | None = Query(None), current_user: User | None = Depends(analysis_current_user)):
    ...
    try:
        ...
        if generate_report and settings.enable_report_generation and report.status == "success":
            ...
            report.report_id, report.report_download_url = report_id, build_artifact_url(f"/api/v1/artifacts/reports/{report_id}", report_id, "report", exercise_id=report.exercise_id or report.exercise)
        ...
        if include_overlay and settings.enable_overlay_generation and report.status == "success" and report.frame_analysis:
            try:
                ...
                logger.warning("Shoulder-abduction overlay generation failed: %s", exc)
        ...
        logger.exception("Unable to complete shoulder-abduction analysis")
        for artifact_path in generated_artifacts:
            artifact_path.unlink(missing_ok=True)
        return error_response(status.HTTP_500_INTERNAL_SERVER_ERROR, "PROCESSING_ERROR", "Unable to complete shoulder-abduction analysis.")
    finally:
