"""Upload endpoint for the rule-based static balance screen."""
...
async def analyze_balance(
    ...
    try:
        ...
        report = balance_analyzer.analyze_landmarks(landmarks, include_frame_data=include_frame_data or include_overlay)
        report = apply_assisted_exercise_fallback("balance", report, landmarks, include_frame_data=include_frame_data or include_overlay)
        ...
        if include_overlay and settings.enable_overlay_generation and report.status == "success" and report.frame_analysis:
            try:
                ...
                logger.warning("Balance overlay generation failed: %s", exc)
                report.limitations.append("Annotated balance preview could not be generated for this analysis.")
        ...
        logger.exception("Unable to complete balance analysis")
        for artifact_path in generated_artifacts:
            artifact_path.unlink(missing_ok=True)
        return error_response(status.HTTP_500_INTERNAL_SERVER_ERROR, "PROCESSING_ERROR", "Unable to complete balance analysis.")
    finally:
