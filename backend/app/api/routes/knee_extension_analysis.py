"""Upload endpoint for the rule-based knee-extension analyzer."""
...
async def analyze_knee_extension(
    ...
    try:
        ...
        if include_overlay and settings.enable_overlay_generation and report.status == "success" and report.frame_analysis:
            try:
                ...
                logger.warning("Knee-extension overlay generation failed: %s", exc)
        ...
        logger.exception("Unable to complete knee-extension analysis")
        for artifact_path in generated_artifacts:
            artifact_path.unlink(missing_ok=True)
        return error_response(
            status.HTTP_500_INTERNAL_SERVER_ERROR, "PROCESSING_ERROR",
            "Unable to complete knee-extension analysis.",
        )
    finally:
