"""Lowered -> raised -> lowered shoulder-flexion repetition state machine."""
...
from .schemas import ShoulderFlexionCountResult, ShoulderFlexionRepEvent
...
def count_shoulder_flexion_reps(angles: list[float], timestamps: list[float] | None = None, frame_indexes: list[int] | None = None, low_confidence_mask: list[bool] | None = None, pose_quality_score: float = 1.0, pose_detection_rate: float = 1.0) -> ShoulderFlexionCountResult:
    if not angles:
        return ShoulderFlexionCountResult()
    ...
    events: list[ShoulderFlexionRepEvent] = []
    ignored = 0
        ...
        if state == "seeking_lowered":
            ...
            if lowered_run >= MIN_PHASE_FRAMES:
                ...
                if raised_pos is not None and angle_range >= MIN_ANGLE_RANGE and valid_duration:
                    events.append(ShoulderFlexionRepEvent(indexes[start], indexes[raised_pos], indexes[position], round(duration, 3) if duration is not None else None, round(minimum, 2), round(maximum, 2), round(angle_range, 2)))
    ...
    if events:
        ...
        range_clarity = mean(min(1.0, event.angle_range / 100) for event in events)
        ...
        confidence = 0.0
    return ShoulderFlexionCountResult(len(events), len(events), ignored, round(max(0.0, min(1.0, confidence)), 3), events, [float(value) for value in durations], phases, transitions, smoothed)
