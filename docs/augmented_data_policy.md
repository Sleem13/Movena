...
- Variants derived from protected holdout videos must not be used for training or counted as independent holdout observations; v2 evaluation filters to real holdout videos only.
...
Augmentation cannot create `squat_fast_uncontrolled` coverage when no reviewed real source exists for that class. It also cannot convert a blank manual rep-count annotation into validated ground truth.
