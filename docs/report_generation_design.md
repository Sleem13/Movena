...
Artifacts expire according to `ARTIFACT_RETENTION_HOURS`, which defaults to 24 hours. Expired files are removed opportunistically whenever artifacts are created or resolved and can be cleaned explicitly with the safe cleanup script. Source uploads are deleted immediately after analysis regardless of success. If report or overlay generation fails, partial artifacts from that request are deleted and the client receives the standard `PROCESSING_ERROR` envelope without internal paths.
