...
The app-supported analyzers are `bodyweight_squat`, `sit_to_stand`, `knee_extension`, `shoulder_abduction`, and `hip_abduction`. Manual selection remains primary; none of the three newer exercises has a trained recognition or biomechanics model.
...
Planned taxonomy labels include `heel_raise`, `balance`, `walking_gait_screen`, `lunge`, and `step_up`. Dataset coverage for these labels does not mean an exercise works in the app.
