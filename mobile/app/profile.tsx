...
import { homeRoute, isTherapist } from "@/src/utils/care";
...
export default function ProfileScreen() {
  ...
  return <Screen><StatusBadge label={user.role} tone="blue" /><Title>{user.full_name || "Movena user"}</Title><Card><Body>{user.email}</Body><Body muted>Session tokens are stored with Expo SecureStore on this device.</Body></Card><PrimaryButton title={isTherapist(user.role) ? "Open patient workspace" : "Open today's plan"} onPress={() => router.replace(homeRoute(user.role))} /><PrimaryButton title="Log out" onPress={async () => { await signOut(); router.replace("/"); }} secondary /><SafetyNotice /></Screen>;
}
