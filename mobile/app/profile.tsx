...
export default function ProfileScreen() {
  ...
  return <Screen><StatusBadge label={user.role} tone="blue" /><Title>{user.full_name || "PhysioVision user"}</Title><Card><Body>{user.email}</Body><Body muted>Session tokens are stored with Expo SecureStore on this device.</Body></Card><PrimaryButton title="View session history" onPress={() => router.push("/history")} /><PrimaryButton title="Log out" onPress={async () => { await signOut(); router.replace("/exercises"); }} secondary /><SafetyNotice /></Screen>;
}
