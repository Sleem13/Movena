...
export default function SessionHistoryScreen() {
  ...
  useEffect(load, [load]);
  if (authLoading) return <AppShell active="history"><Loading label="Checking your account" /></AppShell>;
  if (!user) return <AppShell active="history"><BrandHeader title="History" subtitle="Your saved exercise sessions appear here." /><View style={styles.loginState}><View style={styles.largeIcon}><Ionicons name="lock-closed-outline" size={34} color={colors.primary} /></View><Heading>Log in to view history</Heading><Body muted>Protected session history is available only to your account.</Body><PrimaryButton title="Log in" onPress={() => router.push("/login")} /></View></AppShell>;
  return <AppShell active="history">
    <BrandHeader title="History" subtitle="Review your saved movement sessions." />
    ...
    {loading ? <Loading label="Loading sessions" /> : error ? <ErrorState message={error} action={<PrimaryButton title="Retry" onPress={load} />} /> : !items.length ? <EmptyState title="No saved sessions yet" message="Complete an analysis and save it to see your progress here." /> : items.map((item) => <Pressable key={item.session_id} onPress={async () => { try { setDetail(await getSessionById(item.session_id)); } catch (requestError) { setError((requestError as Error).message); } }} style={({ pressed }) => pressed && styles.pressed}><Card><View style={styles.row}><View style={styles.sessionIcon}><Ionicons name="fitness-outline" size={22} color={colors.primary} /></View><View style={styles.flex}><Heading>{item.exercise_display_name}</Heading><Body>{item.total_reps ?? 0} reps · {item.movement_score ?? "Not scored"}</Body><Body muted>{new Date(item.created_at).toLocaleDateString()}</Body></View><Ionicons name="chevron-forward" size={20} color={colors.text} /></View></Card></Pressable>)}
