"""Create a conservative shoulder-abduction candidate registry for manual review."""
...
DEFAULT_OUTPUT = Path("data/processed/labels/shoulder_abduction_training_candidates.csv")
...
STRONG_TERMS = ("shoulder_abduction", "shoulder abduction", "shoulder-abduction", "lateral arm raise", "arm abduction")


def _is_candidate(row: dict[str, str]) -> tuple[bool, str]:
    if row.get("exercise_id", "").strip().lower() == "shoulder_abduction":
    ...
    if input_path.exists():
        with input_path.open("r", encoding="utf-8-sig", newline="") as source:
            for row in csv.DictReader(source):
                ...
                if not matches:
                    continue
                rows.append({column: row.get(column, "") for column in OUTPUT_COLUMNS} | {"exercise_id": "shoulder_abduction", "requires_manual_review": "true", "training_ready": "false", "candidate_reason": reason, "notes": "Candidate only; exercise identity, view, rep count, and label require human review."})
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("w", encoding="utf-8", newline="") as target:
        writer = csv.DictWriter(target, fieldnames=OUTPUT_COLUMNS)
        writer.writeheader()
        writer.writerows(rows)
    print(f"Shoulder-abduction candidates: {len(rows)}")
